		...
		<el-table-column prop="shepi" label="操作" width="170">
			<template slot-scope="scope">
				<view style="display: flex;">
					...
					<el-button type="warning" v-if="tableData[scope.$index].appealStatus==4" disable>已取消</el-button>
					<el-button type="success" v-if="tableData[scope.$index].appealStatus==2" disable>申诉通过</el-button>
					<el-button type="danger" v-if="tableData[scope.$index].appealStatus==3" disable>审核驳回</el-button>
				</view>
			</template>
		</el-table-column>
	</el-table>
...
export default {
	methods: {
		...
		handleClick(row) {
			console.log(this.tableData[row].appealStatus);
			this.request.post({
				url:'teacher/teacher/discipline/appeal/v1/approval',
				params:{
					 "disciplineAppealId":this.tableData[row].disciplineAppealId ,
